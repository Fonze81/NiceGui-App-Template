...
#
# Observação:
# - A API pública do módulo foi mantida (load_settings/save_settings/apply_settings_to_state)
# - A escrita não "regera" o TOML; ela atualiza somente chaves conhecidas no documento
...
def _ensure_table(root: TOMLDocument | Table, key: str) -> Table:
    """
    ...
    # Se existir com tipo errado (ex.: string), substituímos para restaurar hierarquia mínima.
    table = tomlkit.table()
    root[key] = table
    return table


def _set_nested_value(doc: TOMLDocument, dotted_path: str, value: Any) -> None:
    """
    ...
    cursor: TOMLDocument | Table = doc
    for part in parts[:-1]:
        cursor = _ensure_table(cursor, part)

    cursor[parts[-1]] = value


def _apply_state_to_document(doc: TOMLDocument, state: AppState) -> None:
    """
    ...
    """
    _set_nested_value(doc, "app.name", state.meta.name)
    _set_nested_value(doc, "app.version", state.meta.version)
    _set_nested_value(doc, "app.language", state.meta.language)
    _set_nested_value(doc, "app.first_run", state.meta.first_run)
    _set_nested_value(doc, "app.native_mode", state.meta.native_mode)
    _set_nested_value(doc, "app.port", state.meta.port)

    _set_nested_value(doc, "app.window.x", state.window.x)
    _set_nested_value(doc, "app.window.y", state.window.y)
    _set_nested_value(doc, "app.window.width", state.window.width)
    _set_nested_value(doc, "app.window.height", state.window.height)
    _set_nested_value(doc, "app.window.maximized", state.window.maximized)
    _set_nested_value(doc, "app.window.fullscreen", state.window.fullscreen)
    _set_nested_value(doc, "app.window.monitor", state.window.monitor)
    _set_nested_value(doc, "app.window.storage_key", state.window.storage_key)

    _set_nested_value(doc, "app.ui.theme", state.ui.theme)
    _set_nested_value(doc, "app.ui.font_scale", state.ui.font_scale)
    _set_nested_value(doc, "app.ui.dense_mode", state.ui.dense_mode)
    _set_nested_value(doc, "app.ui.accent_color", state.ui.accent_color)

    # Persistimos como string para interoperabilidade e facilidade de edição.
    _set_nested_value(doc, "app.log.path", _normalize_path_for_toml(state.log.path))
    _set_nested_value(doc, "app.log.level", state.log.level)
    _set_nested_value(doc, "app.log.console", state.log.console)
    _set_nested_value(doc, "app.log.buffer_capacity", state.log.buffer_capacity)
    _set_nested_value(doc, "app.log.rotation", state.log.rotation)
    _set_nested_value(doc, "app.log.retention", state.log.retention)

    _set_nested_value(doc, "app.behavior.auto_save", state.behavior.auto_save)
...
def _build_minimal_document_from_state(state: AppState) -> TOMLDocument:
    ...
    Motivo:
    - No primeiro save não há arquivo anterior (nem comentários a preservar)
    - Geramos estrutura mínima e previsível do template
    """
    doc = tomlkit.document()
    _apply_state_to_document(doc, state)
    return doc
...
# -----------------------------------------------------------------------------
# A API permanece igual para não impactar outros módulos:
# - load_settings(...)-> bool
# - save_settings(...)-> bool
#
# O round-trip é interno: parse TOMLDocument, atualiza chaves conhecidas, dumps.
...
def load_settings(
    *,
    ...
    try:
        doc = _parse_toml_document(path.read_text(encoding="utf-8"))
        apply_settings_to_state(st, doc)
